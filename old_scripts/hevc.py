...
def run_ffmpeg_transcode(scale_filter, bitrate, short_name):
    """Execute a single FFmpeg transcoding sub-process thread wrapper using libx265."""
    ...
    # ADJUSTMENT A: Change the pattern suffix to .m4s for Fragmented MP4 chunks
    ...
    cmd = [
        ...
        # ADJUSTMENT 1: Ensure standard 8-bit pixel format for broad web decoding
        "-pix_fmt", "yuv420p",
        
        # ADJUSTMENT 2: Force proper keyframe constraints directly inside libx265
        "-x265-params", "keyint=60:min-keyint=60:scenecut=0",
        
        "-hls_time", "4",            
        "-hls_playlist_type", "vod", 
        
        # ADJUSTMENT 3: Instruct FFmpeg to build fMP4 segments rather than TS
    ...
    # Track the newly created .m4s slices accurately for our telemetry logs
...
def main():
    ...
    # HEVC/H.265 target ladder profile matrix (target bitrates are cut roughly in half compared to H.264)
    all_possible_profiles = [
        {"name": "1080p", "scale": "1920:-2", "bitrate": "2500k", "bandwidth": 2500000, "res": "1920x1080", "threshold": 1080},
        {"name": "720p",  "scale": "1280:-2", "bitrate": "1250k", "bandwidth": 1250000, "res": "1280x720",  "threshold": 720},
        {"name": "480p",  "scale": "854:-2",  "bitrate": "500k",  "bandwidth": 500000,  "res": "854x480",  "threshold": 480}
    ]
    ...
    if not active_profiles:
        ...
        active_profiles.append({
            "name": "fallback", "scale": "trunc(iw/2)*2:trunc(ih/2)*2", "bitrate": "300k", "bandwidth": 300000, "res": f"scaled_native", "threshold": 0
        })
        
    logger.info(f"Step 4/6: Initiating filtered transcoding pipeline matrix...")
    logger.info(f"Targeting HEVC outputs: {[p['name'] for p in active_profiles]}")
